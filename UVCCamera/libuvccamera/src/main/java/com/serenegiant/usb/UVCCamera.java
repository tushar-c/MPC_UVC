...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
	 ...
	 */
	public void setPreviewSize(int width, int height, final int min_fps, final int max_fps, final int frameFormat, final float bandwidthFactor) {
		if ((width == 0) || (height == 0))
...
//				throw new IllegalArgumentException("Failed to set preview size");
				try {
					JSONObject object = new JSONObject(nativeGetSupportedSize(mNativePtr));
					JSONArray formats = object.getJSONArray("formats");
					JSONObject sizeObject = formats.getJSONObject(0);
					JSONArray sizes = sizeObject.getJSONArray("size");
					String resString = sizes.get(0).toString();
					String resWidth = resString.split("x")[0];
					String resHeight = resString.split("x")[1];
					Log.e(TAG, "setPreviewSize: " + resWidth);
					width = Integer.valueOf(resWidth);
					height = Integer.valueOf(resHeight);
					mCurrentWidth = Integer.valueOf(resWidth);
					mCurrentHeight = Integer.valueOf(resHeight);
